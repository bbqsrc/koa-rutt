...
class Router {
   ...
   * @since 0.4
   *
   * @param {String} path - The URL prefix for path to assets.
   * @param {String} realPath - The path to assets on local file system.
   *
   * @returns {Router} Returns this instance of Router.
   */
  assets(path, realPath) {
    this.route(`${path}/:assetPath(.+)`).middleware.get = function* sendAsset(ctx) {
      yield send(ctx, ctx.params.assetPath, { root: realPath })
    }
